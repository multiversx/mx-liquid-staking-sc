...
use governance_sc_interact::{GovernanceCallsInteract, GovernanceConfig};
...
async fn cs_builtin_run_tests() {
    let mut governance_interactor =
        GovernanceCallsInteract::new(GovernanceConfig::chain_simulator_config()).await;
    ...
    let mut delegation_interactor = DelegateCallsInteract::new(
        delegation_sc_interact::DelegationConfig::chain_simulator_config(),
    )
    .await;
    let validator_1 =
